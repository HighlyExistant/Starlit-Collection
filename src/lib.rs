...
mod tests {
    ...
    fn barebones_vec_test() {
        ...
        let mut host2vec = VkVec::<i32, dyn GeneralAllocator>::new(host_freelist.clone());
        let devvec = VkVec::<i32, dyn GeneralAllocator>::new_zeroed(500, device_freelist.clone()).unwrap();
        for i in 0..500 {
            hostvec.push(i);
            host2vec.push(i);
        }
